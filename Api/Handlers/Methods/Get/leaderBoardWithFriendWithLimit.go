...
func LeaderBoardWithFriendWithLimit(endpoint string, w http.ResponseWriter, r *http.Request) {
	params := r.URL.Query()
	...
	if claims, err := utils.GetClaims(&receiveToken); err == nil {
		...
		if uuid, ok := claims["UUID"].(string); ok {
			data = utils.GetPlayerOnEndpointWithFriend(lim1, lim2, selectedEndpoint, uuid, rawPlayer.SelectedData)
